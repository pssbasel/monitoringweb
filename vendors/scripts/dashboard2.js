...
function getFileProgress() {
    return fetch("https://script.google.com/macros/s/AKfycbyi5NY8PhaUVtClM3aGfeXDtrUPqLt5MYbAgZ_ffYW4EICCcfqqEjSmoEHSZBdv3G66/exec?action=countFiles")
        .then(res => res.json())
        .catch(err => ({ count: 0 }));
}
...
function updateProgress() {
    getFileProgress().then(data => {
        ...
        if (!data || typeof data.count === "undefined") {
            console.error("Data JSON tidak sesuai:", data);
            return;
        }

        const count = data.count;  
        const max = 100; // Target fleksibel, bisa nanti ambil dari JSON juga
